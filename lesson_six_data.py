...
def shape_element(element):
    ...
    if element.tag == "node" or element.tag == "way" :
        ...
        node['pos'] = [lat,lon]
        for item in element:
            if element.tag == "way" and item.tag == "nd":
                noderefs.append(item.attrib['ref'])
            elif item.attrib.has_key('k') and not problemchars.search(item.attrib['k']):
                addarr = item.attrib['k'].split(":")
                if len(addarr) == 2 and addarr[0] == "addr":
                    address[addarr[1]]= item.attrib['v']
                elif len(addarr) == 2:
